...
def get_deduplicated_tokens(logits_batch: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Converts batch of logits into batch most probable tokens and their probabilities.

    Args:
        logits_batch (Tensor): Batch of logits (N x T x V)
    
    Returns:
        Tuple: Deduplicated tokens. The first element is a tensor (token indices) and the second element is a tensor (token probabilities)
...
def make_len_mask(input_tensor: torch.Tensor) -> torch.Tensor:
    return (input_tensor == 0).transpose(0, 1)

def get_len_util_stop(sequence: torch.Tensor, end_index: int) -> int:
